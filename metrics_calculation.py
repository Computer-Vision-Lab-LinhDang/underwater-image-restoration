...
def calculate_metrics_ssim_psnr(generated_image_path, ground_truth_image_path, resize_size=(256, 256)):
    ...
    for img in generated_image_list:
        ...
        generated_image = cv2.resize(generated_image, resize_size, cv2.INTER_LINEAR)

        ground_truth_image = cv2.imread(ground_truth_image)
        ground_truth_image = cv2.resize(ground_truth_image, resize_size, cv2.INTER_LINEAR)
        # calculate SSIM
...
print(calculate_UIQM('demo/non-ref/UIEB/UnderWater').mean())